...
def get_argmax(frame):
    ...
    return np.argmax(hist), hist


def get_argmax2(img):
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    mask = cv2.inRange(hsv, (0, 125, 125), (10, 255, 255))
    red_pixels = cv2.bitwise_and(img, img, mask=mask)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    print()
    print(len(contours[0]))
    print(contours)
    # use a bounding box or minimum enclosing circle algorithm to draw a box or circle around the red object
    for contour in contours:
        x, y, w, h = cv2.boundingRect(contour)
        cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 2)
    hist = np.sum(red_pixels[:,:,2]/255, axis=0)
    return hist, img 
...
def main():
    ...
    while cap.isOpened():
        ...
        hist, img = get_argmax2(frame)
        #cv2.circle(img, (100,100), 10, (0, 255, 0), 10)
        cv2.imshow('Img', img)

        #print(img[100][100])
        #print("newx=", np.argmax(hist))
        


        paddle.x = (np.argmax(hist))*(WIDTH/hist.shape[0])
